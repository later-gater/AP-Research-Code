...
eos1_perm = {"low": 0.1, "low_range": 0.4, "high_range": 1.2, "high": 1.8}
eos2_perm = {"low": 0.4, "low_range": 0.8, "high_range": 3.6, "high": 10.}
robot_growth_func_perm = {"none": dill.dumps(lambda t: 1), "linear": dill.dumps(lambda t: t), "2_exp": dill.dumps(lambda t: 2**t)}
income_inequality_perm = {"high": [45, 10, 5], "mid": [30, 20, 10], "low": [25, 20, 15]}
rate_k_perm = {"low": 3, "high": 9}
rate_z_perm = {"low": 2, "high": 7}


permutations = itertools.product(alpha2_perm.items(), eos1_perm.items(), eos2_perm.items(), robot_growth_func_perm.items(), income_inequality_perm.items(), rate_k_perm.items(), rate_z_perm.items())
